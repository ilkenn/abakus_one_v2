...
**Boncuk Loyalty Program P3A (2026-08-23) — Real Customer Loyalty Data + Premium Boncuklarım Screen,
IMPLEMENTED — pending physical device visual review before checkpoint commit (visually NOT yet
closed).** Replaces the entirely mock customer-facing Boncuklarım screen with a real, server-
authoritative one and makes `lib/features/loyalty/` the canonical owner of the feature. Old mock audit
confirmed: hardcoded `320` balance, bronze/silver/gold tiers with fake 1000/3000 thresholds, a 4-item
mock rewards catalog with a working fake redeem dialog, a client-side `Random()` spin wheel, 3 mock
campaigns, and 3 hardcoded history rows — all removed from the customer-visible surface. The 5 previously
-empty `lib/features/loyalty/` stub files are now populated with the real implementation; the old mock
files (`features/profile/presentation/{screens/loyalty_screen.dart,providers/loyalty_provider.dart}`)
were **not deleted**, per CLAUDE.md's standing no-unilateral-deletion rule — they are now fully
unreferenced and are flagged here for the user's own follow-up deletion decision. A new callable,
`getCustomerLoyaltyHistory` (`functions/src/getCustomerLoyaltyHistory.ts`), is the first customer-safe
read path for `loyaltyLedgerEntries` — gated identically to `getCustomerLoyaltySnapshot`, bounded/
paginated (default 15, max 30, single-field cursor), and returns a narrow sanitized projection
(`eventId`/`type`/`displayBoncukDelta`/`debtAppliedBoncuk`/`occurredAt`/`orderId`) rather than the raw
ledger document — see `BR-LOYALTY-017` for the full reasoning (Firestore rules control *access*, not
*field exposure*; a raw scoped client query, while access-safe, would still leak internal accounting
provenance fields with zero customer-facing meaning). The real, already-existing
`getCustomerLoyaltySnapshot` response (`spendableBalance`/`boncukDebt`/`earningRemainderMinorUnits`) is
rendered for the first time by an actual screen — no client-side balance calculation, no ledger
aggregation on-device, no fallback/mock value. Final screen structure: hero balance card (`AbacusCard`,
real balance + informational TL-equivalent), earning-progress card (`LoyaltyProgressCard`, computed from
the real `earningRemainderMinorUnits` field — e.g. "34 TL / 50 TL", "Sonraki Boncuk için 16 TL kaldı"),
a static how-it-works block, a real ledger-backed Boncuk Movements section (5-row preview + a full
paginated `LoyaltyHistoryScreen`, infinite-scroll), and an honest empty state ("Henüz Boncuk hareketin
yok.") — no fake transactions. Bronze/silver/gold tiers, campaigns, and the rewards/wheel benefits
section are all absent this phase (tiers remain an open product decision; catalog/wheel/tasks remain
unimplemented — nothing fake was left in their place). Debt UX: a nonzero `boncukDebt` renders a calm,
non-alarming explainer ("İade nedeniyle `N` Boncuk sonraki kazanımlarından dengelenecek.") — never
"borçlusun" or legalistic phrasing; spendable balance is never shown negative. Routes preserved: all 5
original entry points (`home_screen.dart` ×3, `profile_loyalty_card.dart`, `profile_screen.dart`) still
open the same `LoyaltyScreen` class, now at its canonical path — only the import statement changed at
each site. A genuine, previously-undetected UI overflow bug was found and fixed during this phase: the
loading skeleton was not wrapped in a scrollable, so its stacked placeholder content overflowed on a
375px-class viewport — fixed by wrapping it in a `SingleChildScrollView`. No `firestore.rules` change
(the new callable uses the Admin SDK). See `docs/decisions.md`'s P3A entry for the full files-changed
list and exact gate totals (Functions 919/919, up from 902 — 17 new `getCustomerLoyaltyHistory` tests;
Firestore Rules 345/345 unchanged; `flutter analyze` 0 issues; `flutter test` 3274/12-skipped/0-failed,
up from 3236). **REAL LOYALTY BALANCE/HISTORY = connected. MOCK BALANCE/TIERS/REWARDS/WHEEL/CAMPAIGNS/
HISTORY = removed from the customer-visible surface. CATALOG/WHEEL/TASKS = still not implemented. REAL
REFUND EXECUTION = still not implemented (P2B). LOYALTY PRODUCTION READY = NO.** Physical device visual
review is required before any checkpoint commit — not yet performed as of this entry.

**Boncuk Loyalty Program P3A Visual Polish + Rate Change (2026-08-24) — IMPLEMENTED, pending a second
physical device visual review before checkpoint commit.** Responds to P3A's first physical review
having FAILED on visual grounds only (functionality was accepted). Visually redesigns
`lib/features/loyalty/presentation/{widgets/abacus_card.dart,widgets/loyalty_progress.dart,
screens/loyalty_screen.dart}` — compact sage-to-primary gradient hero with the real balance as the
sole focal point and a purely decorative bead motif (mirroring `ProfileLoyaltyCard`'s established
technique), a branded gradient progress bar with a percentage badge for "Sonraki Boncuğa", a 3-chip
compact "Nasıl Çalışır?" replacing the old text-list card, and a compact single-row empty-movements
card replacing the generic full-page `EmptyView`. No backend contract, data source, or business logic
changed by the visual portion — every rendered number is still real, sourced from the unchanged P3A
providers; no wheel/rewards/tiers/campaigns were reintroduced; no fake data anywhere. Also implements
a separately locked Boncuk economics rate change in the same task: earning rate 50 TL = 1 Boncuk → **10
TL = 1 Boncuk**, redemption rate 1 Boncuk = 2 TL → **1 Boncuk = 1 TL**
(`BONCUK_EARNING_RATE_MINOR_UNITS_PER_BONCUK` 1000, `BONCUK_REDEMPTION_VALUE_MINOR_UNITS_PER_BONCUK`
100, `functions/src/loyaltyLedger.ts`; mirrored in `LoyaltyAccountSnapshot`'s Flutter constants). The
50%-of-order redemption cap and single-benefit-per-order stacking rule are unchanged. Every rate-
dependent backend test assertion (`loyaltyLedger.test.ts`, `loyaltyOrderEarning.test.ts`,
`loyaltyReversalMath.test.ts`) was individually re-derived against the new rate, including several
tests whose own seeded input values had to change (not just their expected outputs) to still exercise
the same intended scenario at the new rate — see `docs/decisions.md`'s P3A Visual Polish + Rate Change
entry for the full before/after worked-example table and exact gate totals. Redemption itself remains
**NOT YET IMPLEMENTED** (no checkout flow exists — the redemption rate is defined and displayed
informationally only); refund execution remains not implemented (P2B), unchanged by this phase.
**LOYALTY PRODUCTION READY = NO.** Physical device visual review of this second pass is required before
any checkpoint commit — not yet performed as of this entry.

**Boncuk Configurable Loyalty Economics (2026-08-24) — IMPLEMENTED (resolver/read path only; no Admin
write callable or Admin UI, per explicit locked scope).** Responds to a new locked product requirement:
Boncuk economics must not remain permanently hardcoded — a future Admin must be able to change the
earning ratio, redemption value, and maximum redemption percentage per organization. Introduces two new
Firestore collections mirroring the existing ledger/account materialized-view split:
`loyaltyPolicies/{organizationId}` (the current active policy, auto-provisioned to the locked default —
50 TL eligible net spend = 5 Boncuk, 1 Boncuk = 1 TL, maximum 50% of order — on an organization's first
resolution, exactly like `loyaltyAccounts`' own zero-account provisioning) and
`loyaltyPolicyVersions/{organizationId}_{version}` (append-only, immutable policy history). Every
earning/reversal formula in `functions/src/{loyaltyOrderEarning,loyaltyReversalMath}.ts` changed from
importing a fixed rate constant to accepting the resolved rate as a parameter — for the current default
policy this is mathematically identical to the rate already in use, confirmed by the full test suite
passing unchanged. Every `orderEarn` ledger entry now snapshots the exact rate AND policy version that
produced it (`loyaltyPolicyVersion`, new field); a dedicated test proves that changing an organization's
policy (via direct Admin SDK write, since no Admin callable exists yet) affects only new earning going
forward — an already-written historical ledger entry is asserted byte-for-byte unchanged afterward,
never rewritten, never recalculated. A reversal of an old order always uses that order's own
snapshotted rate, never today's policy — locked in `loyaltyReversalMath.ts`'s own parameter doc comment.
Tenant isolation (one organization's policy can never affect another's) and fail-safe corruption
handling (a malformed policy document blocks the operation rather than falling back to a fabricated
rate) are both proven by dedicated tests at the resolver, earning-transaction, and customer-snapshot-
callable layers. `getCustomerLoyaltySnapshot`'s response gains a sanitized `policy` object; the
customer Boncuklarım screen's "50 TL → 5 Boncuk"/"1 Boncuk → 1 TL"/"En fazla %50'si"
(`_HowItWorksCard`) and the "Sonraki Boncuğa" progress denominator now render from these real,
server-resolved fields — `LoyaltyAccountSnapshot`'s rate/redemption fields changed from Flutter
`static const` values to real instance data, with a dedicated widget test proving a non-default policy
renders genuinely different numbers on screen, not just at the model layer. Customer Flutter has no
code path that could write a policy value even if it tried — `LoyaltyGateway`'s interface has no such
method. **No Admin write callable, no Admin UI, no checkout Boncuk redemption were implemented** — all
explicitly out of this task's own locked scope. See `docs/decisions.md`'s Configurable Loyalty Economics
entry for the full architecture, files changed, and exact gate totals. No commit was made.

**Same-day correction (2026-08-24) — the design above was rejected before merge and never reached
`main`.** Four issues: (1) the "`earningSpendMinorUnits` must divide evenly by `earningBoncukAmount`"
constraint above is **removed** — a genuinely non-integer-reducible ratio (locked example: `5000 minor
units → 3 Boncuk`) is now exactly supported via `BigInt` integer math, never floating point, never a
single derived per-Boncuk rate; (2) a **per-policy-version earning epoch model** was added
(`loyaltyAccounts.earningPolicyVersion`) so a policy version change never re-rates a customer's
existing in-progress accumulated spend — only new spend after the change uses the new ratio.
**Semantic cleanup (2026-08-24, second pass)**: "preserved" is split into two distinct, independently-
verified guarantees rather than one blended claim — `OLD_PROGRESS_AUDIT_PRESERVED = YES` (the retired
version's aggregate/remainder/`earningPolicyVersion`, and every ledger entry that produced them, are
never rewritten, deleted, or migrated onto the new version — permanently available for audit; already-
earned Boncuk are unaffected) and `OLD_PROGRESS_CAN_COMPLETE = NO` (despite the above, the abandoned
remainder can never itself convert into a Boncuk once its policy version is retired — no future event
resumes it). The abandoned technical remainder is also confirmed **structurally excluded** from what
the customer is ever shown: `getCustomerLoyaltySnapshot`'s response is always freshly recomputed from
a `0` aggregate when the account's epoch doesn't match the live policy version, never sourced from the
stale stored remainder. Proven by two tests — the existing `loyaltyOrderEarning.test.ts` integration
test using the architect's own locked V1=5000→5 → V2=5000→3 example, plus a new dedicated
`getCustomerLoyaltySnapshot.test.ts` read-path test covering the gap before the customer's next order:
seeds real stale V1 progress against an already-live V2 policy and asserts the response shows a fresh
V2 epoch (never the stale remainder, never a reinterpretation) while the stored account document is
byte-for-byte untouched by the read; (3) every `orderEarn`
ledger entry now snapshots the **raw ratio itself** (`earningSpendMinorUnits`/`earningBoncukAmount`,
replacing the removed single-rate field), not just a version pointer, so history stays interpretable
without reading today's live policy; (4) a new **`loyaltyPolicyBootstraps/{organizationId}`** marker
collection distinguishes genuine first-time policy provisioning from a policy that should already
exist but is missing/corrupted — the latter now fails closed
(`missing-live-policy`/`missing-live-loyalty-policy`) instead of silently recreating the locked default
(which could otherwise unexpectedly reset an already-configured organization's live economics).
`LoyaltyAccountSnapshot`'s derived `earningRateMinorUnitsPerBoncuk` getter is removed (it would
silently floor-truncate a non-reducible ratio); `minorUnitsUntilNextBoncuk` is now a real, required
field populated verbatim from a new server response field. Two mandatory Flutter fixtures — A (50 TL →
5 Boncuk, 1 Boncuk = 1 TL, 50%) and B (50 TL → 3 Boncuk, 1 Boncuk = 0.50 TL, 25%) — are proven correct
end-to-end, including the progress card's non-fixed block-size denominator and sub-1-TL redemption-
value formatting. See `docs/decisions.md`'s Configurable Loyalty Economics entry §11 for the full
corrected architecture, worked examples, and files changed. No commit was made.

**Fractional Entitlement Carry correction (2026-08-24, third pass) — the per-policy-version "earning
epoch" model above (point 2) was itself rejected before merge and never reached `main`.** It never
re-rated old spend, but it FORFEITED the customer's real, economically-earned partial progress by
resetting the account's aggregate to `0` on the next earning event after a policy change —
`OLD_PROGRESS_AUDIT_PRESERVED = YES` was true, but `OLD_PROGRESS_CAN_COMPLETE` was `NO`, which is not
acceptable: both must be `YES`. The corrected design tracks partial progress as an EXACT,
POLICY-INDEPENDENT FRACTION of one Boncuk (`loyaltyAccounts.earningCarryNumerator`/
`earningCarryDenominator`, canonical decimal strings — never `Number`, since a carry denominator can
exceed `Number.MAX_SAFE_INTEGER` after a few policy changes) instead of a currency-denominated,
epoch-scoped aggregate. Every earning event, under whichever policy is active, combines the EXISTING
carry with the new order's own exact fractional entitlement via exact `BigInt` fraction arithmetic
(`functions/src/loyaltyPolicy.ts`'s `combineCarryWithEarning`) — there is no epoch, no reset, and a
policy change never touches an existing account at all. Locked worked example, test-proven: a customer
carries `2/5` (0.40 Boncuk) earned under V1 (`5000→5`); V2 activates (`5000→3`); new V2 spend
contributes exactly `3/5` (0.60 Boncuk); `2/5 + 3/5 = 1` exactly → the customer receives 1 whole
Boncuk, nothing lost, nothing re-rated. `loyaltyReversalMath.ts` was rewritten from a single-aggregate
subtraction formula to a REPLAY-based one (restore the reversed order's own snapshotted carry, re-apply
every later order's own snapshotted spend/ratio, compare against what was originally recorded) — proven
against the exact scenario a naive "subtract the reversed order's own delta" would get wrong: an
earlier order's fractional contribution that helped a LATER order complete a whole Boncuk. **The
`getCustomerLoyaltySnapshot` response shape is byte-for-byte unchanged** (still
`spendableBalance`/`boncukDebt`/`earningRemainderMinorUnits`/`minorUnitsUntilNextBoncuk`/
`lifetimeEarned`/`lifetimeRedeemed`/`policy`) — the exact carry is now always projected fresh against
whichever policy is current, so **zero Flutter changes were required**, confirmed by rerunning
`flutter test` unchanged (3288 passed, 0 failed). Functions emulator suite: 963/963, 0 failed (up from
959). See `docs/decisions.md`'s Configurable Loyalty Economics entry §12 for the full corrected
architecture, the mathematical proof behind the replay-based reversal, and files changed. No commit was
made.

**O(1) reversal correction (2026-08-24, fourth pass) — the REPLAY-based reversal design directly
above was itself rejected before merge.** It was mathematically correct, but operationally UNBOUNDED —
a refund of an old order could require scanning and replaying hundreds or thousands of later ledger
entries; production reversal must never scan the ledger. The corrected design adds a new account field,
`validOrderEntitlementBoncuk` (int, `>= 0` — the currently valid whole Boncuk entitlement from
non-reversed order spend, distinct from `lifetimeEarned`, which never decreases on reversal), maintained
incrementally at zero extra cost (`+= wholeBoncukEarned` on every earning event, the same value already
computed for the ledger/debt logic). A full reversal now reads exactly two things — the account's
CURRENT `validOrderEntitlementBoncuk` + exact carry, and the reversed order's own immutable ledger
entry, looked up directly by its deterministic id — and computes `newExact = (validOrderEntitlementBoncuk
+ carry) − originalContribution` (the original order's own exact fraction, reconstructed from its own
snapshot, never today's policy), splitting the result back into a whole/fractional pair. **No later
ledger entry is ever read.** This is proven, not merely claimed, mathematically identical to the
rejected replay design — by the same telescoping property that already justified the carry model
itself — verified by a dedicated test that computes an identical cross-policy scenario via both the O(1)
formula and an independent reference replay implementation and asserts byte-for-byte identical results.
`orderEligibleNetSpendMinorUnits`/`earningRemainderMinorUnits`/`earningPolicyVersion` (the rejected
epoch design's own fields) remain confirmed absent from the shipped schema — the final account model is
exactly `spendableBalance`/`boncukDebt`/`validOrderEntitlementBoncuk`/`earningCarryNumerator`/
`earningCarryDenominator`/`lifetimeEarned`/`lifetimeRedeemed`/`revision`/`createdAt`/`updatedAt`, no
other loyalty-accounting field. Denominator growth is proven bounded under both same-policy repetition
and small-set policy cycling, with a defensive `10^30` fail-closed ceiling for the pathological case.
**The `getCustomerLoyaltySnapshot` response shape is unaffected for the second correction pass in a
row** — zero Flutter changes required, confirmed by rerunning `flutter test` unchanged (3288 passed, 0
...
**Boncuk Loyalty Program P7-A (2026-08-24) — Reward Catalog Fast Audit + Final Product Model Plan,
CLOSED (audit only, no code changed).** Found two independently-authored, both-dead mock reward
catalogs (the documented `BR-LOYALTY-007` spec: 50/100/200/200; the orphaned `loyalty_provider.dart`'s
different 50/100/150/250) and confirmed nothing reward-catalog-shaped is reachable in the real customer
UI today. Backend already reserves `catalogRedemption`/`catalogRedemptionRestore` ledger-entry types
and a `{entryType, rewardId}` metadata shape — zero writer exists. Found `loyaltyRedemptionRestore.ts`
is hardcoded to `entryType === "boncukRedemption"` specifically — not a zero-touch reuse for a future
catalog-reward restore path. Found real, current price spread within a single menu category (bowl:
430–800 TL) makes category-level entitlement a real arbitrage risk — recommended explicit product
allowlists instead. Confirmed zero nested-subcollection precedent exists anywhere in `firestore.rules`
— the originally-proposed `loyaltyRewardCatalog/{organizationId}/rewards/{rewardId}` shape was rejected
for a flat collection. **Critical economics finding**: the original 50/100/200/200 catalog was priced
against the 1 Boncuk = 2 TL rate in effect at P0-A (2026-08-20); the P3A rate change (already committed,
same-day-dated but from earlier in this project's history) moved redemption to 1 Boncuk = 1 TL while
making earning 5× more generous, a combined ~2.5× swing the reward catalog was never re-priced
against — real current menu prices confirmed the old costs would be roughly 2× too generous if carried
forward unchanged. `OLD_REWARD_COSTS_SAFE_TO_KEEP=NO`, no new costs invented (no COGS data exists to
derive them), flagged for architect decision. Directly informed P7-B's implementation plan.

**Boncuk Loyalty Program P7-B (2026-08-24) — Server-Authoritative Reward Catalog Foundation, CLOSED,
backend foundation only.** Implements P7-A's accepted recommendation. New `loyaltyRewardCatalog`
(live)/`loyaltyRewardCatalogVersions` (append-only immutable) collections, mirroring `loyaltyPolicy.ts`'s
own already-accepted "live doc + version-history collection" pattern. `rewardType = "explicitProductSet"`
only this phase — no category-based automatic eligibility, per P7-A's own arbitrage finding; every
`eligibleProductIds` entry is validated server-side against the real canonical `menuProducts` collection
(exists, same organization) before a reward can be created or updated. `active`/`archived` are
deliberately NOT part of the versioned definition (operational flags, not the historical definition) —
toggling them mutates the live doc directly, never creating a new version. No destructive delete, ever
— "delete" is exclusively `active: false, archived: true`, permanently retained. Trusted admin-service
primitives (`createLoyaltyReward`/`updateLoyaltyRewardByCreatingNextVersion`/`setLoyaltyRewardActive`/
`archiveLoyaltyReward`) are plain functions, deliberately NOT a callable this phase (challenged against
`provisionOrganization`'s own `platformOwner`-authorized-callable precedent and rejected — no reward-
catalog-manager permission exists yet, and inventing one before any real Admin screen consumes it would
be premature permission/UI scope) — invoked today only by the local dev-seed script. New customer read
callable `getCustomerLoyaltyRewardCatalog` mirrors `getCustomerLoyaltySnapshot.ts`'s exact identity
discipline, returns only active/non-archived/currently-valid (server time only)/correct-tenant rewards
as a sanitized DTO; `firestore.rules` denies all direct client access to both collections
unconditionally. New pure `resolveCatalogRewardRedemption` foundation validates eligibility/balance and
returns an immutable proposed snapshot — deliberately NOT built on `calculateBoncukRedemption` (that
function's 50%-of-order-cap math doesn't apply to a fixed-cost item exchange) — **debits nothing, writes
no ledger entry**; that is P7-C's own scope. The four LOCKED initial rewards were re-priced against real
current menu data (not the old 50/100/200/200): İçecek → 70 Boncuk (explicit 4-item beverage allowlist,
every current canonical beverage, all uniformly 80 TL); Çıtırtı Bowl → 420 Boncuk
(`prod_citirti_bowl`); Crispy Chicken Fettuccine → 400 Boncuk (`prod_fettucine_crispy_chicken_alfredo` —
the sole catalog product matching both keywords, a deterministic resolution, not guessed); Falafel Salad
→ 400 Boncuk (`prod_crispy_falafel_salad` — same reasoning, sole Salata-category match). Seeded via
`functions/scripts/seed_dev_loyalty_reward_catalog.mjs` (new `npm run seed:dev-loyalty-reward-catalog`),
run twice against the live `abakus-one-dev` local emulator to prove idempotency, and smoke-tested
end-to-end via a real `getCustomerLoyaltyRewardCatalog` HTTP call. **Still explicitly out of scope**:
Admin UI, general Campaign Engine, checkout/redemption wiring into any order-submission channel, the
`catalogRedemption` ledger writer, any account debit, reward inventory/quantity caps, partial-cash-plus-
reward orders. Gates: Functions build clean; Functions FULL emulator suite **1336/1336, 0 failed** (up
from 1283); Firestore Rules suite **361/361, 0 failed** (up from 354); `flutter analyze`/`flutter test`
not rerun (Flutter untouched).
See `docs/business_rules.md`'s `BR-LOYALTY-007` (marked SUPERSEDED with the economics correction) and
new `BR-LOYALTY-026`, and `docs/decisions.md`'s P7-A/P7-B entries for the full mechanism and files
changed. No commit was made.

**Boncuk Loyalty Program P7-C (2026-08-24) — Atomic Catalog Reward Redemption, Takeaway Only, CLOSED.**
Closes P7-B's own disclosed checkout/redemption-wiring, ledger-writer, and account-debit gaps for
exactly one channel. `submitTakeawayOrder` now accepts an optional `selectedRewardId` (client sends only
the id — server re-resolves cost/title/version from the current canonical reward every time; a forged
client-supplied cost field is structurally inert, there is none). Mutually exclusive with cash Boncuk
redemption, fail-closed with a stable reason if both are requested. A reward makes exactly ONE eligible
cart-product UNIT free — proven exact (not approximate) via a new `freeUnitCount` parameter on
`buildOrderLine` (`takeawayPricing.ts`), since every unit within one order line is priced identically;
because `unitPriceMinorUnits` is already channel-resolved before that function runs, Takeaway's own
+20 TL packaging surcharge is covered automatically for the rewarded unit, with no separate surcharge
logic. `redeemedQuantity` is always exactly 1 regardless of the line's cart quantity; every other unit/
item remains fully, normally priced; an order with no reward/Boncuk selection prices exactly as before.
The order snapshot (`catalogReward: {rewardId, rewardVersion, title, boncukCost, redeemedProductId,
redeemedQuantity, coveredValueMinorUnits, rewardCatalogVersionId}`) is written once and never re-derived
from the live catalog afterward — a later live-reward edit can never rewrite an already-placed order's
history. Account debit and the deterministic `catalogRedemption` ledger entry reuse the exact existing
accounting/ledger machinery (`deriveLoyaltyLedgerEntryId`), atomically with order creation — no second
accounting engine. `loyaltyRedemptionRestore.ts`'s single consumer is generalized (not duplicated) to
restore whichever of `boncukRedemption`/`catalogRedemption` exists for an order (never both — an
invariant violation if it ever finds both, fails closed rather than guessing), via the same debt-first
primitive and idempotency discipline; cash-Boncuk restore behavior is byte-for-byte unchanged. The
rewarded unit earns zero Boncuk — verified (not assumed) that the existing earning-basis function already
handles this correctly via the pricing fix above, requiring zero logic change, only a clarifying comment.
Flutter: new `CatalogRewardCard` on the Takeaway checkout screen reads the real server-authoritative
catalog (`loyaltyRewardCatalogProvider` → `getCustomerLoyaltyRewardCatalog`, P7-B) — no mock reward
source; mutual exclusivity with `BoncukRedemptionCard` is enforced by REMOVING the other card from the
widget tree entirely (never merely disabling); the success screen shows a compact, fully server-confirmed
summary ("`<title>` ödülü kullanıldı" / "`X` Boncuk kullanıldı" / "Ücretsiz ürün: `<product>`" /
"Karşılanan tutar: `Y` TL"). **A genuine regression was found and fixed by the gate run itself**: the
first `loyaltyRedemptionRestore.ts` restructuring let the redemption-family lookup run before the order
document's own existence/status/identity validation, silently letting "nothing to restore anyway" win
over a real anomaly — caught by 2 failing pre-existing tests, fixed by restoring the original check order
(order validation always first), re-verified clean on a full rerun. **Still explicitly out of scope**:
Delivery/Reservation-preorder catalog-reward redemption, the general Campaign Engine, any Admin UI. Gates:
Functions build clean; Functions FULL emulator suite **1366/1366, 0 failed** (up from 1336 — first run
1364/1366 before the restore-ordering fix); Firestore Rules FULL suite **361/361, 0 failed** (unchanged —
`firestore.rules` not touched this phase); full-repo `flutter analyze` clean; `flutter test` **3376
passed / 12 skipped / 0 failed**. See `docs/business_rules.md`'s new `BR-LOYALTY-027`, and
`docs/decisions.md`'s P7-C entry for the full mechanism and files changed. No commit was made.

**Boncuk Loyalty Program P7-C.1 (2026-08-24) — Channel-Scoped Reward Foundation Microfix, CLOSED.** A
schema/validation microfix on P7-B/P7-C, not a new redemption mechanism — Delivery/Reservation-preorder
still cannot redeem catalog rewards. Every Reward Catalog entry now carries a required, non-empty
`eligibleChannels: CanonicalCommercialChannel[]` from a new closed vocabulary,
`CANONICAL_COMMERCIAL_CHANNELS = ["dineIn", "takeaway", "delivery", "reservationPreorder"]`
(`loyaltyRewardCatalog.ts`) — the documented shared boundary a future Campaign Engine will also import
(only the constant/boundary is locked this phase; no campaign mechanics were built). Deliberately coarser
than a real order's own `channel` field (`"dineInQr"`/`"dineInStaff"` both collapse into the single
commercial `"dineIn"` concept). `sanitizeEligibleChannels` rejects empty/missing/unsupported values and
deterministically normalizes duplicates/reordering into one canonical stored array shape.
`eligibleChannels` is part of the versioned reward definition (like `boncukCost`/`eligibleProductIds`) —
changing it via `updateLoyaltyRewardByCreatingNextVersion` creates a new immutable version; every prior
version remains byte-for-byte unchanged. `submitTakeawayOrder.ts` now validates a selected reward's
`eligibleChannels` includes `"takeaway"` (the server's own real, hardcoded channel — never client-
supplied; a forged client `channel` field has zero effect) BEFORE any pricing/build work, rejecting with
the new stable `catalogReward/channel-not-eligible` reason and touching nothing (no debit, no ledger, no
order) on failure — mirrored, defense-in-depth only, inside `resolveCatalogRewardRedemption`'s own pure
resolver (new `orderChannel` parameter, reusable by a future Delivery/Reservation integration). The order
snapshot (`CatalogRewardOrderSnapshot`) gained `orderChannel: CanonicalCommercialChannel`, preserved
immutably at redemption time — a later reward channel-scope change can never rewrite an already-placed
order's history; the redeemed reward VERSION's own immutable `eligibleChannels` is automatically
preserved too, as a direct consequence of the schema extension. The four LOCKED initial rewards are
re-seeded with all four canonical channels (the starting configuration only — a future Admin can
independently narrow each reward's channels with zero redesign). No Flutter file was touched — nothing
required it (the existing Flutter mapper tolerates the new unread field; server-side validation already
fails closed without client awareness of the vocabulary). Gates: Functions build clean; targeted test run
(5 reward-catalog files + the unmodified `loyaltyRedemptionRestore.test.ts` as a regression check)
**128/128, 0 failed**; Functions FULL emulator suite **1391/1391, 0 failed** (up from 1366); Firestore
...
**Server-Authoritative Campaign Engine P8-C (2026-08-25) — Takeaway checkout integration, and P8-C.1
(2026-08-25) — Delivery checkout integration.** Wires P8-B's own foundation-only, previously-unused
modules (`campaignEngine.ts`/`campaignScheduling.ts`/`campaignPricing.ts`/`campaignUsage.ts`/
`benefitExclusivity.ts`) into real order submission — first `submitTakeawayOrder.ts` (P8-C), then
`submitDeliveryOrder.ts` (P8-C.1), reusing every shared module verbatim with zero duplication (per each
task's own explicit "do not build a duplicate campaign engine" instruction). A customer can now select
one real, server-authoritative campaign (`selectedCampaignId` only — never a client-computed discount)
for a takeaway or delivery order: the server resolves eligibility (tenant/active/archived/channel/
schedule/minimum-basket/product-category targeting) from trusted state, computes the exact discount via
a two-pass line-building flow, reserves global/per-customer usage atomically with order creation
(proven concurrency-safe by N-simultaneous-submissions-against-a-limit-of-K tests for both channels),
and releases that usage exactly once on a qualifying terminal event (rejected/cancelled/refunded) via
the new, channel-agnostic `campaignUsageRestore.ts` consumer — never on successful completion. All six
Admin-facing campaign types (`percentageDiscount`/`fixedAmountDiscount`/`freeProduct`/`buyXGetY`/
`productDiscount`/`categoryDiscount`) are proven working end-to-end on both channels. `enforceBenefitExclusivity()`
now actually enforces "one benefit per order" across Boncuk/catalogReward/campaign on both channels,
replacing each channel's own previously-hand-rolled two-benefit check. Loyalty earning required zero
campaign-specific code on either channel — proven, not just designed, by dedicated earning tests
(discounted amount excluded, remaining paid amount earns normally). A genuine P8-B latent bug
(recurring-schedule campaigns were permanently unreadable due to a read/write sanitizer mix-up) was
found via P8-C's own testing and fixed. Flutter: a single, real `activeCampaignsProvider`-backed
`CampaignSelectionCard` widget is reused by both `TakeawayCheckoutScreen` and `DeliveryCheckoutScreen`
(mutually exclusive with Boncuk cash/catalog reward selection on both screens); the server-confirmed
success screen and the order-detail historical view both show the campaign summary sourced exclusively
from the canonical re-read `Order` snapshot, never a pre-submit estimate. A real cross-channel reuse bug
(the shared card widget hardcoded its channel-eligibility filter to `"takeaway"`, silently hiding every
delivery-eligible campaign) was found via this phase's own Flutter test run and fixed with a required
`commercialChannel` parameter. **Explicitly not built**: `reservationPreorder.ts`/`submitDineInOrder.ts`
campaign redemption, Admin UI, coupon-code redemption — deliberately deferred. Gates: Functions build
clean for both phases; Functions FULL emulator suite (JDK 21) **1647/1647, 0 failed** (up from 1567 —
43 new takeaway-campaign tests + 37 new delivery-campaign tests); Firestore Rules — not run, neither
phase touched `firestore.rules`/`firestore.indexes.json` (confirmed via `git status`); full-repo
`flutter analyze` clean for both phases; `flutter test` result recorded in the final P8-C.1 report. See
`docs/business_rules.md`'s updated `BR-PROMO-008`, and `docs/decisions.md`'s combined P8-C/P8-C.1 entry
for the full mechanism and files changed. No commit was made in either phase.

**Server-Authoritative Campaign Engine P8-C.2 (2026-08-25) — Reservation preorder checkout
integration, CLOSED.** Wires the same unmodified P8-B shared engine into `submitReservation.ts`'s
existing `if (parsedPreorder)` block (via `reservationPreorder.ts`'s two-pass line-building helpers),
completing checkout campaign redemption for all three customer-facing commercial channels (takeaway,
delivery, reservation preorder) — `submitDineInOrder.ts` remains the one explicitly deferred channel.
A dedicated pre-implementation audit (all 8 reservation lifecycle files) confirmed **no structural
blocker**: the channel-agnostic `onOrderTerminalFailureOrRefund.ts` trigger + `campaignUsageRestore.ts`
consumer P8-C/P8-C.1 already built required **zero new backend wiring** to correctly release campaign
usage on every genuine reservation-preorder terminal transition — only new tests to prove it. Two
genuine simplifications unique to this channel: `ReservationPolicy.timezone` was already resolved
earlier in the same transaction (no extra `branches/{branchId}` read needed for the schedule check, as
takeaway/delivery both required), and reservation preorder has no channel surcharge and no separate
restaurant-operational minimum-order threshold to reconcile against `minimumBasketMinorUnits`. All six
Admin-facing campaign types proven working; `enforceBenefitExclusivity()` now replaces this channel's
own previously-hand-rolled two-benefit stacking check too (same `benefit/stacking-not-allowed` reason
change as P8-C/P8-C.1, requiring the same one-line pre-existing-test fix). Terminal-release mapping was
audited exhaustively and encoded into tests exactly as found — 6 confirmed-terminalizing paths (reject,
customer self-cancel, staff post-release cancel, no-show pre/post-release, refund, response-timeout
sweep) release usage; 4 confirmed-non-terminalizing paths (proposal decline, proposal-expiry sweep,
staff-cancel-on-already-released, confirm/accept) explicitly proven to NOT release it, per the task's
own "do not invent a release" instruction. `confirmedTime` changes (via an accepted alternative-time
proposal) never re-price an already-applied campaign discount — structurally guaranteed by
`buildPreorderConfirmationPatch`'s own patch shape (no `pricing`/`campaign` field exists in it at all),
proven end-to-end by a dedicated propose→accept snapshot-equality test. Flutter: `ReservationFlowScreen`
reuses the same `CampaignSelectionCard`/`CampaignSuccessSummary` widgets `TakeawayCheckoutScreen`/
`DeliveryCheckoutScreen`/`OrderSuccessScreen` already established (`commercialChannel:
'reservationPreorder'`, never hardcoded to another channel — the exact P8-C.1-fixed bug not repeated
here); `ReservationConfirmationScreen`/`reservation_detail_screen.dart`'s `_PreorderStatusCard` show the
server-confirmed/historical campaign summary sourced from two new `ReservationPreorderSummary` fields,
parsed from the same `campaign` Firestore field the other two channels' `Order.campaign` already uses.
Gates: Functions build clean; Functions FULL emulator suite (JDK 21) **1689/1689, 0 failed** (up from
1647 — 42 new reservation-campaign tests). The first run showed one failure in a pre-existing, unrelated
`campaignUsage.test.ts` concurrency test; a dedicated final-gate cleanup pass root-caused it to a known
Firestore-emulator/Admin-SDK transaction-retry wording gap (already fixed once before in this codebase,
`orderEarnReversal.test.ts`'s `withTransientEmulatorTransportRetry`) and applied the same established,
narrowly-scoped fix — never a skip or loosened assertion — reaching a clean 1689/1689 (see
`docs/decisions.md`'s full P8-C.2 entry for the exact root cause and fix). Firestore Rules — not run,
`firestore.rules`/`firestore.indexes.json` untouched (confirmed via `git diff --stat`, zero output);
full-repo `flutter analyze` clean; `flutter test` **3444 passed, 12 skipped, 0 failed** (up from 3434
passed + 12 skipped — 10 new tests). See `docs/business_rules.md`'s updated `BR-PROMO-008`, and
`docs/decisions.md`'s P8-C.2 entry for the full mechanism and files changed. No commit was made.

**Server-Authoritative Campaign Engine P8-C.3 (2026-08-25) — Dine-in checkout integration, CLOSED.**
Wires the same unmodified P8-B shared engine into `submitDineInOrder.ts`'s own single order-creation
transaction, completing checkout campaign redemption for all four customer-facing commercial channels
(takeaway, delivery, reservation preorder, dine-in) — only Admin UI and coupon-code redemption remain
deferred. Dine-in is the one channel with a genuine anonymous-guest identity (table-QR guests) to
reason about for campaigns; a real policy conflict was found during audit — `campaignUsage.ts`'s own
P8-B doc comment had designed (never implemented) a permissive "guest may use a non-customer-limited
campaign" rule, while `submitTakeawayOrder.ts`'s own actually-shipped P8-C guest path had already chosen
a stricter blanket exclusion instead — and was surfaced explicitly rather than silently resolved.
**LOCKED decision (confirmed with the product owner before implementation): anonymous/table-QR guests
cannot use Campaigns, cash Boncuk redemption, or Catalog Rewards, and never earn Loyalty, full stop** —
mirroring dine-in's own pre-existing guest exclusions on the other two benefits exactly, now formally
documented in `BR-LOYALTY-030`. `getCustomerActiveCampaigns` remains unchanged (still open to both
identity types — a best-effort listing hint, submission is the final authority);
`submitDineInOrder.ts` gained exactly one new guest-specific rejection
(`campaign/requires-customer-identity`), mirroring its own pre-existing catalog-reward guest rejection
verbatim. Terminal-release required zero new backend wiring (dine-in's own simpler, linear staff-driven
lifecycle already writes `status` through the same generic helper every channel uses). All six
campaign types proven working; `enforceBenefitExclusivity()` now replaces this channel's own previously
-hand-rolled stacking check too, with the pre-existing "cash Boncuk always rejected" rule provably
still winning first for a combined request (proven by test, not merely assumed). A genuine test-
environment latency characteristic (not a code defect) was root-caused via a temporary diagnostic run
before being fixed with a realistic, still-bounded timeout. Two further, unrelated pre-existing flaky
tests (`deliveryOrderLifecycle.test.ts`, `functions.test.ts`) and one from this same phase family's own
P8-C.2 predecessor (`submitReservationCampaign.test.ts`'s response-timeout-sweep test) surfaced only
once the full suite grew to 1729 tests — each root-caused individually (a tight polling timeout; a
genuine watch-establishment race in a test helper, closed properly this time; a single-call assumption
against a genuinely global, cross-tenant batch sweep function) and fixed deterministically, never
skipped or loosened — see `docs/decisions.md`'s full P8-C.3 entry §6 for the exact mechanism of each.
Flutter: `DineInCheckoutScreen` reuses `CampaignSelectionCard` (`commercialChannel: 'dineIn'`) and
`OrderSuccessScreen`'s existing `_hasCampaignSummary` gate (broadened to include the already-existing
`_isDineIn` flag, a one-line change); `order_detail_screen.dart`'s historical campaign display needed
zero changes, confirmed already channel-agnostic. Gates: Functions build clean; Functions FULL emulator
suite (JDK 21) **1729/1729, 0 failed** (up from 1689 — 40 new dine-in-campaign tests); Firestore Rules —
not run, `firestore.rules`/`firestore.indexes.json` untouched (confirmed via `git diff --stat`, zero
output); full-repo `flutter analyze` clean; `flutter test` **3449 passed, 12 skipped, 0 failed** (up from
3444 passed + 12 skipped — 5 new tests). See `docs/business_rules.md`'s updated `BR-PROMO-008`/
`BR-LOYALTY-030`, and `docs/decisions.md`'s P8-C.3 entry for the full mechanism and files changed. No
commit was made.

**Server-Authoritative Campaign Engine P8-D (2026-08-26) — Final Closure Audit, CLOSED.** An audit-only
task (explicitly: "do NOT add new Campaign features") re-verifying, end-to-end across all four
commercial channels together, everything P8-B through P8-C.3 had each only proven per-channel in
isolation: single shared engine with zero per-channel duplication; server authority over
definition/version/eligibility/channel/schedule/branch-timezone/basket/discount/usage/pricing; atomic,
concurrency-safe global and per-customer usage reservation; the immutable order-level campaign
snapshot confirmed read by `order_detail_screen.dart` (the one shared order-detail screen for every
channel) instead of any live campaign re-lookup; ONE ORDER = MAXIMUM ONE BENEFIT via the one shared
`enforceBenefitExclusivity()` call site per channel; post-campaign-value Loyalty earning; the shared,
channel-agnostic, idempotent `campaignUsageRestore.ts` terminal-release consumer (never fires on
successful completion); the LOCKED dine-in guest-exclusion policy; org-scoped tenant isolation (no
`branchId` on `CampaignDefinition`); all 5 campaign Firestore collections confirmed still
`allow read, write: if false`; `campaignAdminService.ts` confirmed a sound, unimplemented-as-UI
foundation for a future Admin/POS phase. **Two genuine defects found and fixed, both documentation-only
(no behavior change)**: `campaignUsage.ts` and `getCustomerActiveCampaigns.ts` still presented the
superseded, more-permissive P8-B guest-campaign design as current guidance in their own doc comments —
both corrected with an explicit "SUPERSEDED (P8-C.3)" note pointing at `BR-LOYALTY-030`.
`docs/decisions.md`'s own original P8-B paragraph was left historically intact with an added
correction beneath it. Two further stale "not yet wired into any channel" comments
(`campaignUsageRestore.ts`, `benefitExclusivity.ts`) were also corrected to reflect all-four-channel
adoption. **Three orphaned/dead files reported, not deleted**: `checkout_screen.dart` (zero real
callers, still contains hardcoded fake `ABAKUS10`/`ILKSIPARIS`/`UCRETSIZ` coupon logic — highest
severity since it's the one with live fake logic, though itself unreachable from real navigation);
`featured_content_section.dart`/`campaigns_screen.dart` (real data source, no mock fallback, but
unmounted from any reachable navigation tree). Zero TODO/FIXME/HACK, zero duplicate reason
strings/enums/inconsistent channel names, no further doc contradictions found. Gates re-run fresh this
phase (not cited from a prior phase): Functions build clean; Functions FULL emulator suite (JDK 21,
isolated `firebase emulators:exec`) **1729/1729, 0 failed** — identical count to P8-C.3, confirming zero
regressions; `flutter analyze` clean; `flutter test` **3449 passed, 12 skipped, 0 failed** — identical
count to P8-C.3; `firestore.rules`/`firestore.indexes.json` confirmed byte-identical to the pre-audit
baseline; `git status` confirmed every changed file is Campaign-related or documentation, no unrelated
