...
| P2-001/P2-002 cleanup — permanent package identifiers | DONE | The temporary `com.example.abakus_one_v2` (Android) / `com.example.abakusOneV2` (iOS/macOS) identifiers used during Sprint 1 provisioning are replaced with the permanent production identifier **`com.abakus.one`** (Android `applicationId`/`namespace`, iOS/macOS `PRODUCT_BUNDLE_IDENTIFIER`) on **all three** Firebase projects, including production. Firebase package/bundle IDs are immutable once an app is registered, so this required registering 6 new apps (Android + iOS/macOS-shared, × 3 projects) under `com.abakus.one` and regenerating every FlutterFire options file (`lib/firebase_options.dart`, `_development.dart`, `_staging.dart`) and native config file (`google-services.json` ×3, `GoogleService-Info.plist` ×3) against them — re-verified against Gradle's own `process<Flavor>DebugGoogleServices` tasks, all passing. **The original 6 app registrations under the `com.example.*` identifiers (2 per project × 3 projects, including production) are now orphaned** — left in place, not deleted, matching the same conservative handling as `abakus-one-dev-4bf2e` below; flagged for the user to remove from each project's Firebase Console at their convenience. Windows/Web/Linux registrations are untouched (they don't carry a Firebase-relevant native bundle identifier in this codebase). Linux's and Windows' own non-Firebase identifier strings (`linux/CMakeLists.txt`'s `APPLICATION_ID`, `windows/runner/Runner.rc`'s company/product strings) were deliberately left as `com.example`/`abakus_one_v2` — out of the requested scope (Android Application ID / iOS Bundle ID only), flagged here rather than silently expanded. |
