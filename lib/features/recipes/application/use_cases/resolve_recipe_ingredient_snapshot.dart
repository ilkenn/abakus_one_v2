...
import '../../domain/recipe_line_flattener.dart';
import '../identity/recipe_ingredient_snapshot_id_generator.dart';

/// Flattens a [RecipeVersion]'s lines (via [RecipeLineFlattener]) and
/// persists the result as [RecipeIngredientSnapshot] rows tied to
/// that one, fixed `recipeVersionId` — Phase 7 (`docs/decisions.md`
/// ADR-024). manager+ (`PosAuthorizedAction.manageRecipes`) —
/// resolving exposes full recipe composition, which is
/// confidentiality-gated the same way creating one is.
class ResolveRecipeIngredientSnapshot {
  ResolveRecipeIngredientSnapshot({
  ...
  Future<List<RecipeIngredientSnapshot>> call({
    ...
    final resolved = await _flattener.flatten(version.lines);
